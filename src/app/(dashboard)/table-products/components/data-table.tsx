...
export default function TransactionsTable() {
    const [allData, setAllData] = useState<Transaction[]>(MOCK_TRANSACTIONS);
    ...
    const handleSort = (field: keyof Transaction) => (direction: 'asc' | 'desc' | null) => {
        ...
        return allData.filter(item => {
            ...
            const matchesFilters = Object.entries(filters).every(([key, selectedValues]) => {
                ...
                switch (key) {
                    ...
                    default: {
                        const itemValue = item[key as keyof Transaction]?.toString() || "";
    ...
    const getUniqueOptions = (field: keyof Transaction): string[] => {
        switch (field) {
            case 'roleId':
    ...
    const handleDeleteClick = (transaction: Transaction) => {
        setTransactionsToDelete(transaction);
        setDeleteOpen(true);
    };

    const handleDelete = () => {
        if (!transactionsToDelete) return;
        if (transactionsToDelete.name.includes('selected items')) {
            ...
            setAllData(prev => prev.filter(u => u.id !== transactionsToDelete.id));
        }
        setDeleteOpen(false);
    ...
    const handleAddSubmit = (e: React.FormEvent) => {
        ...
        const newTransaction: Transaction = {
            id: allData.length > 0 ? Math.max(...allData.map(t => t.id)) + 1 : 1,
            name: addForm.name,
            email: addForm.email,
        ...
        setAllData(prev => [...prev, newTransaction]);
    ...
    const handleEditClick = (transaction: Transaction) => {
        ...
        setEditFormData({
            ...
            roleId: transaction.roleId.toString(),
            departmentId: transaction.departmentId.toString(),
            jabatanId: transaction.jabatanId.toString(),
        });
        setEditOpen(true);
    };
        ...
        <div className="space-y-4">
            <div className="md:flex justify-between space-y-4 md:space-y-0">
                ...
                <div className="md:flex justify-between gap-2 items-center space-y-4 md:space-y-0">
                    <div className="flex justify-end flex-wrap md:flex-nowrap items-center gap-2">
                        ...
                        {/* Tambah Transaction Dialog */}
                        <Dialog open={open} onOpenChange={setOpen}>
                            <DialogTrigger asChild>
                                <Button variant="outline" size="lg" className="cursor-pointer">
                                    <FilePlus /> Transactions
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="max-h-[80vh] w-full overflow-y-auto">
                                <DialogHeader>
                                    <DialogTitle>Tambah Data Transactions</DialogTitle>
                                </DialogHeader>
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <DialogContent className="max-h-[80vh] w-full overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>
                            {transactionsToDelete?.name?.includes('selected items') ? "Delete Selected Transactions" : "Delete Transaction"}
                        </DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                        <p>Apakah kamu yakin akan menghapus data ini?</p>
                        <p className="font-medium">{transactionsToDelete?.name}</p>
